...
public readonly record struct PenPoint(
    ...
    /// actually need.</para>
    /// <para>How fine it actually is depends entirely on the backend, and the spread is four
    /// orders of magnitude. Drawn on by hand, WM_POINTER and WinUI both resolve to the
    /// <b>microsecond</b> and Wintab to the millisecond, each giving one distinct timestamp per
    /// point. WPF is coarser in kind rather than degree: a whole batch of points shares one
    /// timestamp. <see cref="PenConventions.Timestamp"/> names the clock and carries the
