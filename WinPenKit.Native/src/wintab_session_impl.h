...
private:
    ...
    // pkTime's wrap is handled in handle_packet by anchoring against GetTickCount64, which
    // needs no state carried between packets -- the two fields that used to hold the detector's
    // running high word and last reading are gone with it. The origin that makes anchoring
    // possible was measured on 13 Sep 2026; see WintabEpochProbe.
